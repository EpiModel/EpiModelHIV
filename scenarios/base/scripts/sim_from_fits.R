...
fit.c<- readRDS("/net/proj/SHAMPnetdat/model-fits/archive/cohab-conc-A.rds")
fit.p<- readRDS("/net/proj/SHAMPnetdat/model-fits/archive/pers-conc-A.rds")
fit.i<- readRDS("/net/proj/SHAMPnetdat/model-fits/archive/ot-a0.rds")
...
param <- param_shamp(data.params, 
                     URVI.prob = 1, 
                     UIVI.prob = 1,
                     mean.test.HI.f.int = 40,
                     mean.test.W.f.int = 40)

init <- init_shamp()
control <- control_shamp(nsteps = 40, save.other = c("attr", "trans.el"), verbose = TRUE)

netsim(est_f, param, init, control)
