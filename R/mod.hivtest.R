...
hivtest_msm <- function(dat, at) {
  ...
  elig <- which((diag.status == 0 | is.na(diag.status)) &
                prepStat.la == 0 & prepStat == 0 & late.tester == 0)
  ...
  eligNeg <- which((diag.status == 0 | is.na(diag.status)) &
                   prepStat.la == 0 & prepStat == 0 &
                   status == 0 & late.tester == 1)
  ...
  idsTstPrEP.la <- which((diag.status == 0 | is.na(diag.status)) &
                         prepStat.la == 1 &
                         tsincelntst >= prep.tst.int)

  tstAll <- c(idsTstGen, idsTstLate, idsTstAIDS, idsTstPrEP, idsTstPrEP.la)
