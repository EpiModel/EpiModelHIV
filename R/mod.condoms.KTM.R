...
condoms_KTM <- function(dat, at) {
  ...
  for (type in c("main", "pers", "inst")) {
    ...
    if (type == "main") {
      ...
      cond.always.het <- dat$attr$cond.always.main.het
  ...
  ##VI.
    ...
    sex.p1 <- sex[elt.vi[,1]]
    sex.p2 <- sex[elt.vi[,2]]
    cond.always.p1 <-cond.always.het[elt.vi[,1]]
    cond.always.p2 <-cond.always.het[elt.vi[,2]]
    

    num.het <- (sex.p1 == "F") + (sex.p2 == "F")
   
    cond.prob[cond.always.p1==0 & cond.always.p2==0] <- cond.B.prob.het
    #doi:10.1016/s0277-9536(96)00258-4
    cond.prob[cond.always.p1==1 | cond.always.p2==1] <- 1
