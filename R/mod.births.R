...
setBirthAttr_msm <- function(dat, at, nBirths.B, nBirths.W) {
  ...
  dat$attr$stage.time.af.ndx[newIds] <- rep(0, nBirths)
  dat$attr$stage.time.af.dx[newIds] <- rep(0, nBirths)
  dat$attr$stage.time.chronic.ndx[newIds] <- rep(0, nBirths)
  dat$attr$stage.time.chronic.dx.yrone[newIds] <- rep(0, nBirths)
  dat$attr$stage.time.chronic.dx.yrstwotolate[newIds] <- rep(0, nBirths)
