...
prevalence.mard <- function(dat, at) {
  ...
  if (at == 1) {
    ...
    dat$epi$nBirths <- dat$epi$nBirths.B <- dat$epi$nBirths.W <- rep(NA, nsteps)
    dat$epi$dth.gen <- dat$epi$dth.dis <- rep(NA, nsteps)
    ...
    dat$epi$tx.naive <- dat$epi$tx.naive.B <- dat$epi$tx.naive.W <- rep(NA, nsteps)
    dat$epi$tx.full.supp <- dat$epi$tx.full.supp.B <- dat$epi$tx.full.supp.W <- rep(NA, nsteps)
    ...
    if (prevfull == TRUE) {
      ...
      dat$epi$tx.part.supp <- dat$epi$tx.part.supp.B <-
        dat$epi$tx.part.supp.W <- rep(NA, nsteps)
      ...
      dat$epi$tx.influx.full <- dat$epi$tx.influx.full.B <-
        dat$epi$tx.influx.full.W <- rep(NA, nsteps)
      ...
      dat$epi$tx.influx.part <- dat$epi$tx.influx.part.B <-
        dat$epi$tx.influx.part.W <- rep(NA, nsteps)
