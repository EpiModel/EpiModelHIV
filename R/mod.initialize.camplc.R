...
init_status_msm <- function(dat) {
  ...
  time.since.inf <- ceiling(runif(length(selected.all), max = max.inf.time))
  inf.time[selected.all] <- 1 - time.since.inf
  tx.status[selected.all] <- 0
  cum.time.on.tx[selected.all] <- 0
  cum.time.off.tx[selected.all] <- time.since.inf

  stage[selected.all[time.since.inf <= vlar.int]] <- 1
  stage[selected.all[time.since.inf > vlar.int & time.since.inf <= vl.acute.int]] <- 2
  stage[selected.all[time.since.inf > vl.acute.int & time.since.inf <= vldo.int]] <- 3
  stage[selected.all[time.since.inf > vldo.int]] <- 4

  stage.time[selected.all][stage[selected.all] == 1] <- time.since.inf[stage[selected.all] == 1]
  stage.time[selected.all][stage[selected.all] == 2] <- time.since.inf[stage[selected.all] == 2] -
                                                   vlar.int
  stage.time[selected.all][stage[selected.all] == 3] <- time.since.inf[stage[selected.all] == 3] -
                                                  vl.acute.int
  stage.time[selected.all][stage[selected.all] == 4] <- time.since.inf[stage[selected.all] == 4] -
                                                  vldo.int

  vl[selected.all] <- (time.since.inf <= vlar.int) * (vlap * time.since.inf / vlar.int) +
  ...
  tx.init.time.B.asmm <- twind.int + dat$param$last.neg.test.B.int.asmm + 1 / dat$param$tx.init.B.prob.asmm
  tx.init.time.W.asmm <- twind.int + dat$param$last.neg.test.W.int.asmm + 1 / dat$param$tx.init.W.prob.asmm

  # Stage for Blacks MSM
...
reinit_msm <- function(x, param, init, control, s) {
  ...
  if (any(c("param", "control", "nwparam", "epi", "attr", "temp" , "riskhist",
    "el", "p", "riskh") %in% names(x)) == FALSE) {
