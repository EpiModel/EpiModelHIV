    ...
    if (x$control$verbose == TRUE) {
      if (!interactive()) {
        ...
        if (verbose.int > 0 && (at %% verbose.int == 0)) {
          ...
          incid.hiv <- round(x$epi$ir100[at], 3)
          incid.gc <- round(x$epi$ir100.gc[at], 3)
          incid.ct <- round(x$epi$ir100.ct[at], 3)
          prev.syph <- round(x$epi$prev.syph[at], 3)
          prev.pssyph <- round(x$epi$prev.primsecosyph[at], 3)
          prev.pssyph.hivpos <- round(x$epi$prev.primsecosyph.hivpos[at], 3)
          prev.pssyph.hivneg <- round(x$epi$prev.primsecosyph.hivneg[at], 3)
          ...
          cat("\nP and S Syphilis Prevalence in HIV+: ", prev.pssyph.hivpos, sep = "")
          cat("\nP and S Syphilis Prevalence in HIV-: ", prev.pssyph.hivneg, sep = "")
