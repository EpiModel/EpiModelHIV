...
sti_trans_msm <- function(dat, at) {
  ...
  dat$epi$incid.rgc.hivneg[at] <- length(which(status[idsInf_rgc] == 0))
  dat$epi$incid.ugc.hivneg[at] <- length(which(status[idsInf_ugc] == 0))
  dat$epi$incid.rgc.hivpos[at] <- length(which(status[idsInf_rgc] == 1))
  dat$epi$incid.ugc.hivpos[at] <- length(which(status[idsInf_ugc] == 1))
  ...
  dat$epi$incid.rct.hivneg[at] <- length(which(status[idsInf_rct] == 0))
  dat$epi$incid.uct.hivneg[at] <- length(which(status[idsInf_uct] == 0))
  dat$epi$incid.rct.hivpos[at] <- length(which(status[idsInf_rct] == 1))
  dat$epi$incid.uct.hivpos[at] <- length(which(status[idsInf_uct] == 1))
  dat$epi$incid.syph[at] <- length(idsInf_syph)
  dat$epi$incid.syph.hivneg[at] <- length(which(status[idsInf_syph] == 0))
  dat$epi$incid.syph.hivpos[at] <- length(which(status[idsInf_syph] == 1))
  ...
  # Add times inf (temporary)
  if (is.null(dat$epi$gc.timesInf)) {
    dat$epi$gc.timesInf <- rep(NA, dat$control$nsteps)
    dat$epi$ct.timesInf <- rep(NA, dat$control$nsteps)
    dat$epi$sti.timesInf <- rep(NA, dat$control$nsteps)
  }

  dat$epi$gc.timesInf[at] <- mean(dat$attr$rGC.timesInf + dat$attr$uGC.timesInf, na.rm = TRUE)
  dat$epi$ct.timesInf[at] <- mean(dat$attr$rCT.timesInf + dat$attr$uCT.timesInf, na.rm = TRUE)
  dat$epi$sti.timesInf[at] <- mean(dat$attr$rGC.timesInf + dat$attr$uGC.timesInf + dat$attr$rCT.timesInf + dat$attr$uCT.timesInf, na.rm = TRUE)

  # Risk group-specific
  dat$epi$incid.rgc.tttraj1[at] <- length(which(dat$attr$tt.traj.gc.hivneg[idsInf_rgc] == 1)) +
    length(which(dat$attr$tt.traj.gc.hivpos[idsInf_rgc] == 1))
  dat$epi$incid.ugc.tttraj1[at] <- length(which(dat$attr$tt.traj.gc.hivneg[idsInf_ugc] == 1)) +
    length(which(dat$attr$tt.traj.gc.hivpos[idsInf_ugc] == 1))
  dat$epi$incid.rct.tttraj1[at] <- length(which(dat$attr$tt.traj.ct.hivneg[idsInf_rct] == 1)) +
    length(which(dat$attr$tt.traj.ct.hivpos[idsInf_rct] == 1))
  dat$epi$incid.uct.tttraj1[at] <- length(which(dat$attr$tt.traj.ct.hivneg[idsInf_uct] == 1)) +
    length(which(dat$attr$tt.traj.ct.hivpos[idsInf_uct] == 1))
  ...
  dat$epi$incid.rgc.tttraj2[at] <- length(which(dat$attr$tt.traj.gc.hivneg[idsInf_rgc] == 2)) +
    length(which(dat$attr$tt.traj.gc.hivpos[idsInf_rgc] == 2))
  dat$epi$incid.ugc.tttraj2[at] <- length(which(dat$attr$tt.traj.gc.hivneg[idsInf_ugc] == 2)) +
    length(which(dat$attr$tt.traj.gc.hivpos[idsInf_ugc] == 2))
  dat$epi$incid.rct.tttraj2[at] <- length(which(dat$attr$tt.traj.ct.hivneg[idsInf_rct] == 2)) +
    length(which(dat$attr$tt.traj.ct.hivpos[idsInf_rct] == 2))
  dat$epi$incid.uct.tttraj2[at] <- length(which(dat$attr$tt.traj.ct.hivneg[idsInf_uct] == 2)) +
    length(which(dat$attr$tt.traj.ct.hivpos[idsInf_uct] == 2))
...
sti_recov_msm <- function(dat, at) {
  ...
  rgc.asympt.rate <- dat$param$rgc.asympt.rate
  ugc.asympt.rate <- dat$param$ugc.asympt.rate
  gc.tx.int <- dat$param$gc.tx.int
  gc.ntx.int <- dat$param$gc.ntx.int

  rct.asympt.rate <- dat$param$rct.asympt.rate
  uct.asympt.rate <- dat$param$uct.asympt.rate
  ...
  recovRGC_asympt_ntx <- idsRGC_asympt_ntx[which(rbinom(length(idsRGC_asympt_ntx), 1, rgc.asympt.rate) == 1)]
  recovUGC_asympt_ntx <- idsUGC_asympt_ntx[which(rbinom(length(idsUGC_asympt_ntx), 1, ugc.asympt.rate) == 1)]
  ...
  if (!is.na(gc.ntx.int)) {
    ...
    recovRGC_sympt_ntx <- idsRGC_sympt_ntx[which(rbinom(length(idsRGC_sympt_ntx), 1, rgc.asympt.rate) == 1)]
    recovUGC_sympt_ntx <- idsUGC_sympt_ntx[which(rbinom(length(idsUGC_sympt_ntx), 1, ugc.asympt.rate) == 1)]
  }
  ...
  recovRCT_asympt_ntx <- idsRCT_asympt_ntx[which(rbinom(length(idsRCT_asympt_ntx), 1, rct.asympt.rate) == 1)]
  recovUCT_asympt_ntx <- idsUCT_asympt_ntx[which(rbinom(length(idsUCT_asympt_ntx), 1, uct.asympt.rate) == 1)]
  ...
  if (!is.na(ct.ntx.int)) {
    ...
    recovRCT_sympt_ntx <- idsRCT_sympt_ntx[which(rbinom(length(idsRCT_sympt_ntx), 1, rct.asympt.rate) == 1)]
    recovUCT_sympt_ntx <- idsUCT_sympt_ntx[which(rbinom(length(idsUCT_sympt_ntx), 1, uct.asympt.rate) == 1)]
  }
