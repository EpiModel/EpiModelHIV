...
sti_trans_msm <- function(dat, at) {
  ...
  dat$epi$incid.gc.hivneg[at] <- length(which(status[unique(c(idsInf_rgc,idsInf_ugc))] == 0))
  dat$epi$incid.gc.hivpos[at] <- length(which(status[unique(c(idsInf_rgc,idsInf_ugc))] == 1))
  ...
  dat$epi$incid.ct.hivneg[at] <- length(which(status[unique(c(idsInf_rct,idsInf_uct))] == 0))
  dat$epi$incid.ct.hivpos[at] <- length(which(status[unique(c(idsInf_rct,idsInf_uct))] == 1))
